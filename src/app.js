...
function closeCategoryDropdown(except=null){document.querySelectorAll(".category-select-wrap.open").forEach(wrap=>{if(wrap===except)return;wrap.classList.remove("open");const button=wrap.querySelector(".category-select-button");if(button)button.setAttribute("aria-expanded","false");});}
