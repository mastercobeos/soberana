...
// Single-product Product schema for a detail page.
export function productJsonLd({ product, locale, path }) {
  const p = localizeProduct(product, locale);
  ...
  return {
    ...
    name: p.title,
    image: `${SITE_URL}${p.image}`,
    description: p.description || p.intro?.[locale] || p.intro || undefined,
    ...
    offers: {
      ...
      availability: 'https://schema.org/InStock',
      url,
      seller: { '@type': 'Organization', name: 'Soberana Solutions' }
    }
  };
}
